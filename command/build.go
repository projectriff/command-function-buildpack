...
func (b Build) Build(context libcnb.BuildContext) (libcnb.BuildResult, error) {
	...
	e, ok, err := pr.Resolve("riff-command")
	if err != nil {
		return libcnb.BuildResult{}, fmt.Errorf("unable to resolve riff-command plan entry\n%w", err)
	}
	if !ok {
		return result, nil
	}
	...
	result.Processes = append(result.Processes,
		libcnb.Process{Type: "command-function", Command: command},
